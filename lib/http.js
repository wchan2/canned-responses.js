...
;(function(globals) {
  ...
  var Http = function(contentType) {
    if (!(this instanceof Http)) {
      throw new Error('Http must be called as a constructor: new Http(<contentType>)');
    }

    if (!contentType) {
      throw new Error('Http must be called with a string parameter: new Http(<contentType>);')
    }
